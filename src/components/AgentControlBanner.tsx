/**
 * Agent Control Banner — explains read-only vs full agent control per household
...
export function AgentControlBanner({
  ...
  const isReadOnly = mode !== "agent_control";

  const handleActivate = async () => {
    setActivating(true);
    setError(null);
    try {
      await activateAgentControl(householdId);
          ...
          {!showConfirm ? (
            ...
            <div className="rounded-lg border border-amber-600/40 bg-amber-950/30 px-4 py-3">
              ...
              <p className="mt-1 text-sm text-amber-100/90">
                The agent will be able to set operating modes on your {householdId.includes("tesla") ? "Tesla" : "inverter"} system.
